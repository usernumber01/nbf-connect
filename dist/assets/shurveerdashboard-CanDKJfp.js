import{n as e,t}from"./modulepreload-polyfill-CwuZNuQA.js";/* empty css               */import{S as n,_ as r,b as i,f as a,h as o,n as s,p as c,r as l,u,x as d}from"./config-Dd28I3sq.js";import{a as f,i as p,t as m}from"./auth-B-zOuKI2.js";function h(e,t){return f(n=>{n&&n.emailVerified?e(n):t?t():window.location.href=`login.html`})}var g=e((()=>{m()}));t((()=>{g(),m(),l(),u(),document.addEventListener(`DOMContentLoaded`,()=>{h(t=>{e(t)},()=>{window.location.href=`login.html`});let t=document.getElementById(`sidebar-logout`),n=document.getElementById(`mobile-logout`),r=async e=>{e.preventDefault();try{await p()}catch(e){console.error(`Logout failed`,e),alert(`Failed to logout. Please try again.`)}};t&&t.addEventListener(`click`,r),n&&n.addEventListener(`click`,r)});async function e(e){try{let r=await a(n(s,`users`,e.uid)),i={};r.exists()?i=r.data():console.warn(`No user profile found in Firestore.`),t(i),f(i),y(i),await Promise.all([_(i),v(e.uid)]);let o=document.getElementById(`complete-profile-btn`);o&&o.addEventListener(`click`,()=>{o.textContent=`Saving...`,o.disabled=!0,setTimeout(()=>{i.mobile=`9876543210`,i.state=`Delhi`,i.orgType=`ncc`,i.skills=[`Leadership`,`First Aid`],i.photoURL=`photo.jpg`,i.resumeURL=`resume.pdf`,i.verified=!0,t(i),f(i),_(i),document.getElementById(`profile-action`).classList.add(`hidden`)},1e3)})}catch(e){console.error(`Error initializing dashboard:`,e),alert(`Unable to load data. Please refresh.`)}}function t(e){let t=document.getElementById(`welcome-name`),n=document.getElementById(`welcome-badges`);t.textContent=`Welcome back, ${e.displayName||`Shurveer`}!`,t.classList.remove(`skeleton`,`sk-title`);let r=``;e.verified?r+=`<span class="badge green"><i class="fas fa-check-circle"></i> Verified Shurveer</span>`:r+=`<span class="badge amber"><i class="fas fa-clock"></i> Verification Pending</span>`,e.orgType&&(r+=`<span class="badge orange"><i class="fas fa-sitemap"></i> ${e.orgType.toUpperCase()}</span>`),e.state&&(r+=`<span class="badge white"><i class="fas fa-map-marker-alt"></i> ${e.state}</span>`),n.innerHTML=DOMPurify.sanitize(r)}function f(e){let t=0;e.displayName&&(t+=10),e.mobile&&(t+=10),e.state&&(t+=10),e.orgType&&(t+=10),e.skills&&e.skills.length>0&&(t+=10),e.photoURL&&(t+=15),e.resumeURL&&(t+=15),(e.certificateStatus===`pending`||e.certificateStatus===`verified`)&&(t+=20);let n=document.getElementById(`profile-percent-text`),r=document.getElementById(`profile-progress-bar`),i=document.getElementById(`profile-action`);n.textContent=`${t}%`,n.classList.remove(`skeleton`,`sk-text`),r.style.width=`${t}%`,t>=100?r.style.backgroundColor=`var(--status-green)`:t>=50?r.style.backgroundColor=`var(--status-amber)`:r.style.backgroundColor=`var(--status-red)`,t<80?i.classList.remove(`hidden`):i.classList.add(`hidden`)}async function _(e){let t=document.getElementById(`opp-feed`),n=document.getElementById(`stat-opps`);if(!e.state||!e.orgType){t.innerHTML=`<p style="color: var(--text-muted); padding: 1rem;">Please complete your profile (State and Org Type) to see targeted opportunities.</p>`,n.textContent=`0`,n.classList.remove(`skeleton`,`sk-text`);return}try{let a=await c(r(d(s,`opportunities`),i(`state`,`==`,e.state),i(`orgType`,`==`,e.orgType),o(6)));if(n.textContent=a.size.toString(),n.classList.remove(`skeleton`,`sk-text`),a.empty){t.innerHTML=`<p style="color: var(--text-muted); padding: 1rem;">No opportunities in your area yet. Check back soon!</p>`;return}let l=``;a.forEach(e=>{let t=e.data();l+=`
            ...
            `}),t.innerHTML=DOMPurify.sanitize(l)}catch(e){console.error(`Error fetching opportunities:`,e),t.innerHTML=`<p style="color: var(--status-red);">Unable to load opportunities. Please refresh.</p>`,n.textContent=`-`,n.classList.remove(`skeleton`,`sk-text`)}}async function v(e){let t=document.getElementById(`app-feed`),n=document.getElementById(`stat-apps`);try{let a=await c(r(d(s,`applications`),i(`applicantId`,`==`,e),o(3)));if(n.textContent=a.size.toString(),n.classList.remove(`skeleton`,`sk-text`),a.empty){t.innerHTML=`<p style="color: var(--text-muted); padding: 1rem;">You haven't applied to any opportunities yet.</p>`;return}let l=``;a.forEach(e=>{let t=e.data(),n=`status-pending`,r=`Pending`;t.status===`reviewed`&&(n=`status-reviewed`,r=`Reviewed`),t.status===`accepted`&&(n=`status-accepted`,r=`Accepted`),t.status===`rejected`&&(n=`status-rejected`,r=`Rejected`);let i=`Recently`;t.appliedDate&&(i=(t.appliedDate.toDate?t.appliedDate.toDate():new Date(t.appliedDate)).toLocaleDateString()),l+=`
                <div class="app-item">
                    <div>
